...
from ml_stock_selector.prediction import rank_alpha_risk_raw_predictions_sql, rank_raw_predictions_sql
...
def test_alpha_risk_prediction_ranking_uses_absolute_rank_as_active_compatibility(tmp_path):
    con = init_ml_db(tmp_path / "ml.duckdb")
    raw = pd.DataFrame(
        [
            {
                ...
                "run_id": "run",
                "fold_id": "wf",
                "score_version": "v2_alpha_ret5d_risk_only",
                "feature_set_id": "vpa_d_sequence",
                "horizon_d": 5,
                "absolute_model_id": "abs",
    ...
    rank_alpha_risk_raw_predictions_sql(con, "run", "wf", "v2_alpha_ret5d_risk_only")

    rows = con.execute(
        """
        select code, model_id, active_model_id, absolute_rank_pct, active_rank_pct, risk_rank_pct, trade_score_v2
        from ml_predictions_daily
        order by code
        """
    ).fetchall()
    assert rows[0] == ("000001.SZ", "alpha_risk:abs:risk", None, 0.0, 0.0, 0.0, 0.0)
    assert rows[1] == ("000002.SZ", "alpha_risk:abs:risk", None, 1.0, 1.0, 1.0, 1.0)
