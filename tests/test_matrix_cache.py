...
def test_build_fold_matrix_cache_joins_labels_metadata_and_excludes_bse(tmp_path):
            ...
            tradeability_rows.append(
                {
                    ...
                    "limit_band": "limit_20pct" if code.endswith(".BJ") else "limit_10pct",
                    "generated_at": "t",
                }
            )
    ...
    assert y_abs.shape == (2,)
    assert group.sum() == x_train.shape[0]
    assert metadata["is_bse"].fillna(False).eq(False).all()
    assert set(metadata["can_buy_next_open"].fillna(False).astype(bool)) == {False, True}
    assert set(metadata["limit_band"]) == {"limit_10pct"}
    assert set(metadata["code"]) == {"000001.SZ", "000002.SZ"}
    assert manifest["train_rows"] == 2
    assert manifest["train_filter_can_buy_next_open"] is False


def test_build_fold_matrix_cache_can_filter_training_rows_by_next_open_buyability(tmp_path):
    con = init_ml_db(tmp_path / "ml.duckdb")
    feature_rows = []
    label_rows = []
    tradeability_rows = []
    for code, can_buy in [("000001.SZ", True), ("000002.SZ", False)]:
        feature_rows.append(
            {
    ...
    cache = build_fold_matrix_cache(
        ...
        horizon_d=5,
        label_base="from_next_open",
        universe_config={"train_filter_can_buy_next_open": True},
        cache_root=tmp_path / "cache",
        batch_size=2,
    )
    ...
    assert manifest["train_filter_can_buy_next_open"] is True
