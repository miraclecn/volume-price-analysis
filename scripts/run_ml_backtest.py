...
def main() -> None:
    ...
    try:
        ...
        constraints = PortfolioConstraints(min_trade_score=-999.0)
        if bool(config.ml_v2["trade_score_v2_enabled"]):
            scored = score_candidates_v2(add_liquidity_score(add_context_score(preds)))
            targets = allocate_weights(construct_portfolio_targets_v2(scored, constraints, "default"), 0.05, 0.10, True)
        else:
            scored = score_candidates(add_liquidity_score(add_context_score(preds)))
            targets = allocate_weights(construct_portfolio_targets(scored, constraints, "default"), 0.05, 0.10, True)
